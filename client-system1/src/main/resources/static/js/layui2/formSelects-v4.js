...
(function (layui, window, factory) {
        ...
        if (searchUrl) {
            ...
            if (!ajaxConfig.beforeSearch || (ajaxConfig.beforeSearch && ajaxConfig.beforeSearch instanceof Function && ajaxConfig.beforeSearch(id, searchUrl, inputValue))) {
                ...
                fs.clearid = setTimeout(() => {
                    if (ajaxConfig.linkage) {
                        this.ajax(id, searchUrl, inputValue, true, ajaxConfig.linkageWidth, true);
                    } else {
     ...
     * @param labelValues
     */
    Common.prototype.ajaxLabelValue = function (id, labelValues) {
        if(!id || !labelValues || labelValues.length<=0) return;
        let ajaxConfig = ajaxs[id] ? ajaxs[id] : ajax;
        ...
        $.ajax({
            ...
            beforeSend: () => {
                let loadElem = labelElem.find('.label-loading-msg');
                if(!loadElem[0]) {
                    labelElem.append("<span class='label-loading-msg' style='padding-right: 5px; background: #ccc; border: 1px solid #ccc;' >loading...</span>");
                } else {
                    loadElem.text("loading...");
                }
                this.changePlaceHolder(labelElem);
            },
            success: (res) => {
                ...
                if (res.code != 0) {
                    labelElem.find('.label-loading-msg').text("loading label falure!");
                } else {
            ...
            error: (err) => {
                labelElem.find('.label-loading-msg').text("loading label error!");
                console.log(err);
            }
        });
     ...
     * 重命名ajax的请求参数
     */
    Common.prototype.renameAjaxParam = function(id, ajaxData){
        let ajaxConfig = ajaxs[id] ? ajaxs[id] : ajaxs[id] = $.extend(true, {}, ajax);
        let page = ajaxData.page;
        let size = ajaxData.size;
        let keyword = ajaxData.keyword;
        delete ajaxData.page;
        delete ajaxData.size;
        delete ajaxData.keyword;
        ajaxData[ajaxConfig.paramMap.page] = page;
        ajaxData[ajaxConfig.paramMap.size] = size;
        ajaxData[ajaxConfig.paramMap.keyword] = keyword;
        return ajaxData;
    }
    /**
     * 下拉/搜索下拉数据请求
    ...
    Common.prototype.ajax = function (id, searchUrl, inputValue, isLinkage, linkageWidth, isSearch, ajaxData) {
        ...
        //是否需要对ajax添加随机时间
        //ajaxData['_'] = Date.now();
        let extendData = $.extend(true, {}, ajaxData);
        this.renameAjaxParam(id, extendData);
        $.ajax({
            type: ajaxConfig.type,
            headers: ajaxConfig.header,
            url: searchUrl,
            data: ajaxConfig.dataType == 'json' ? JSON.stringify(extendData) : extendData,
            success: (res) => {
                if (typeof res == 'string') {
                    try {
                        res = JSON.parse(res);
    ...
    Common.prototype.renderData = function (id, dataArr, linkage, linkageWidth, isSearch, ajaxData) {
        ...
        if (linkage) {
            ...
            // reElem.find(`.${INPUT}`).css('display', 'none');
            return;
        }
    ...
    Common.prototype.renderBtns = function (id, show, right) {
        let quickBtn = [];
        let dl = $(`dl[xid="${id}"]`);
        let dlWidth = dl.prev().width();
        dlWidth < 300 && (dlWidth = 300);
        quickBtn.push(`<div class="${CZ_GROUP}" show="${show}" style="max-width: ${dlWidth - 54}px;">`);
    ...
    Common.prototype.renderSelectDropdown = function (id, placeholder, select) {
        ...
        if (data[id].config.btns.length) {
            setTimeout(() => {
                ...
                let dlWidth = dl.prev().width();
                dlWidth < 300 && (dlWidth = 300);
                dl.find(`.${CZ_GROUP}`).css('max-width', `${dlWidth - 54}px`);
            }, 10)
            elemArr.push([
    ...
    Common.prototype.one = function (target) {//一次性事件绑定
        $(target ? target : document).off('click', `.${FORM_TITLE}`).on('click', `.${FORM_TITLE}`, (e) => {
            let othis = $(e.target),
                ...
                dl = title.next(),
                id = dl.attr('xid');
            //清空非本select的input val by imlzw 为什么要清空？，暂注释
    ...
    Common.prototype.removeAll = function (id, isOn, skipDis) {
        ...
        // if (dl.find('.xm-select-linkage')[0]) {//针对多级联动的处理
            ...
            //     do {
            //         pid = vs[index++];
            //         li = dl.find(`.xm-select-linkage-group${index}:not(.xm-select-linkage-hide) li[value="${pid}"]`).removeClass(THIS);
            //     } while (li.length && pid != undefined);
            // });
            // return;
        // }
     ...
     * @returns {*}
     */
    Select4.prototype.value = function (id, typeOrVals, isAppend) {
        if (typeof id != 'string') {
            return [];
        }
        let fs = data[id];
        ...
        // 获取控件值数据
        if (typeof typeOrVals == 'string' || typeOrVals == undefined) {
        ...
        if (common.isArray(typeOrVals)) {
            ...
            if (isAdd) {
                fs.values.forEach((val, index) => {
                    existInValues[val.val] = true;
                });
            }
            let ajaxConfig = ajaxs[id] ? ajaxs[id] : ajax;
            let unLabelValue = []
            typeOrVals.forEach((val, index) => {
                if ((val!=null && val!=undefined) && (!existInValues[val] || fs.config.repeat)) {
                    // let fs = data[id];
                    // let dlElem = $(`dl[xid=${id}]`);
                    // if(dlElem && !dlElem.find(".loading")[0]){
                    //     dlElem.append("<span class='loading'></span>");
    ...
    Select4.prototype.data = function (id, type, config) {
        ...
        if (type == 'local') {
            ...
            if (config.initValues && config.initValues.length > 0) {
                this.value(id, config.initValues);
            } else {
                this.value(id, []);
            }
        }
